...
The RP2-D patch helper now checks `0005` before applying it. On a clean post-`0004` tree where the historical `0005` context overlaps, it applies the reviewable `0006` layered snapshot instead; this keeps the immutable old patch untouched while making the pinned runner reproducible from a clean checkout.
